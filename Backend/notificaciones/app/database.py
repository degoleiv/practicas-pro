...
from modules.notificaciones import MicrosoftGraph
print(getenv("IP_MONGODB"))
client = AsyncIOMotorClient(f'mongodb://{getenv("IP_MONGODB")}')
emailNotifi = MicrosoftGraph()
database = client.notificaciondb
collection = database.notificacion


async def get_all_notifi():
    ...
    #     files.append(Notification(**document))
    return files


async def get_one_notifi(id):
...
async def get_search_notifi(regex):
    files = []
    import re
...
async def update_notifi(id: str, data):
...
async def delete_notifi(id: str):
    await collection.delete_one({'_id': ObjectId(id)})
    return True


async def create_notifi(data ):
   
    response = emailNotifi.email("practicaspro2024@hotmail.com")
    if response.ok:

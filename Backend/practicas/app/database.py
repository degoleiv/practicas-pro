...
database = client.incripsciondb
collection = database.inscripcion
...
async def get_all_incripscion ():
    ...
    async for document in cursor:
        incripscion.append(Incripscion(**document))
    return incripscion

async def get_one_incripscion (query):
    incripscion = {}
    try :
        incripscion = await collection.find_one(query)
        incripscion['_id'] = str(incripscion['_id'])
